...
Despite the various degrees of freedom in terms of embedding and replica selection, 
 ...
 As we will see, several problems variants can be solved in different ways (but at different runtimes).
...
In order to solve the $\RS+\MA+\BW$ problem, 
we first construct
...
We start by observing that $\RS+\CC+\MA+\BW$ boils down to
$\RS+\MA+\BW$ in the absence of $\FP$: due to the unique paths
...
First, we construct the artificial graph described above and compute 
an (integer) min-cost-max-flow solution on the normalized and extended graph $\Tree^*$.
Subsequently, 
we reconstruct the replica assignment from the flow solution.
The idea of this path reconstruction is as follows: From the well-known min-cost-max-flow integrality
theorem~\cite{integrality}, we know that there exists an integral optimal solution of the min-cost-max-flow on $\Tree^*$; by
our construction, the 
paths of this flow solutions directly yield the node-replica pairs. 

Now, while a simple algorithm such as the successive shortest path algorithm~\cite{successive_shortest_path_complexity}
would directly give us such an integral solution (in polynomial time), faster algorithms exist
to solve the min-cost-max-flow problem~\cite{mincostmaxflow-1,mincostmaxflow-2}; however, while these algorithms provide cost-optimal solutions, 
these solutions may not be integral yet, and hence, a rounding and path decomposition approach is needed.
Concretely, we use the following approach 
in order to identify a matched chunk-node pair:
we construct an integral flow allocation by (repeatedly) picking
two arbitrary paths of the same length and non-integral, non-zero flow,
and assign the combined flow to only one of the two paths.
...
and costs, due to the integer flow theorem~\cite{integrality},
the resulting min-cost-max-flow solution will result in integer flow allocations.
In order to see why fractional solutions are sufficient for our algorithm to work,
let us examine our rounding strategy in more detail: 
due to the existence of an optimal integral solution, if there is at least one fractional
path $p_1$ left (with flow strictly larger then 0 and strictly smaller than 1), there must also be at least one other fractional path 
$p_2$ left of the same length;
a different length would contradict optimality, as moving flow to the shorter path
would strictly reduce costs. 
Now, by moving the flow of $p_1$ to $p_2$ (or vice versa), the overall cost will not be increased;
moreover, as a result, at least one of the two paths now has an integral flow (namely the path of flow 0).
Therefore, we have just reduced the number of non-integer paths by one, and the proof follows
by induction.

The runtime of our algorithm consists of two parts: construction of $\Tree^*$ 
and computation of the min-cost-max-flow computation. The construction of $\Tree^*$
takes time linear in the substrate size, and this runtime is asymptotically
dominated by the flow computation. 
The state-of-the-art min-cost-max-flow algorithms are based on double-scaling
methods~\cite{mincostmaxflow-1}  or minimum mean-cost cycle algorithms~\cite{mincostmaxflow-2}, 
taking time $O(n_S^2 \log C \log\log U)$ and $O(n_S^5 \log n_S)$ respectively, where $C$ is the maximal
link cost and where $U$ is the maximal link capacity in the substrate.
