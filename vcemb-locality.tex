...
($\leftarrow$) Let's take any feasible solution $\Sol$ to $I$. We construct a solution $\Sol'$ to $I'$ in the following way:
\begin{enumerate}
  \item We place $n$ nodes in $n$ {\TripleGadgets} (one per gadget) that correspond to triples in $\Sol$. We match each of such nodes to chunks in the gadget it is placed, as well as one arbitrary chunk in {\UnqSubtree}.
  \item In each {\ElGadget} that correspond to element $e$, we place $\deg(e) - 1$ nodes and match them to arbitrary chunks in this gadget, which is not yet matched in any {\TripleGadget}
\end{enumerate}

We can observe that every chunk type was processed. By simple calculations we see that $\Sol'$ indeed has cost $\leq \Thr$.

($\rightarrow$)
Let's take any feasible solution $\Sol'$ to $I'$.
...
\noindent \textbf{The substrate network.}  The network is reduced
network from previous proof (no \UnqSubtree, no unique chunks in
Elements Subtree).  Chunks are placed the same way, with an exception
that we have no unique chunk types.
...
\begin{lemma}
  ...
  chunk it processes in every feasible solution), and $\CostCom =
  1$.
...
Next, we show how to precisely control the number of nodes in any
subtree.
...
\begin{lemma}
  ...
  every feasible solution we have exactly $Q$ machines in $T$ (and the
...
\begin{proof}
  ...
  Let's assume by contrary that we feasible solution $\Sol$ to $I$ and
  call $y_i$ the number of nodes spawned in $S_i$. We know that
  $\sum_i y_i = Q$.

  Let's take any $i$ such that $y_i \neq x_i$.  From lemma
  \ref{lem:bandwidth1} we know that
  $y_i \leq x_i \vee y_i \geq \numNodes - x_i$.

\end{proof}

\begin{obs}
  \label{obs:nodes-match-cover}
  Using lemma \ref{lem:bandwidth2} with $T$ being the whole tree, and
...
\begin{obs}
  ...
  Using lemma \ref{lem:bandwidth2} with $T$ being the {\CoverSubtree},
  and $Q = \sum_e (\deg(e)-1)$ we conclude that in every feasible
  solution the number of nodes spawned in {\ElGadget} of element $e$
  is exactly $\deg(e)-1$.
...
\begin{proof}
  ...
  {\TripleGadgets} are active. What remains to be shown is that there are no half-active {\TripleGadgets} in the solution of cost $\leq \Thr$.

  By observations \ref{obs:nodes-match-cover} and \ref{obs:deg-min-one} we know that the cost of communication of nodes in {\CoverSubtree} is the same for every feasible solution (let's name it $P$). We also know that the cost of communication between nodes in {\CoverSubtree} and {\MatchSubtree} is the same for every feasible solution (let's name it $Q$). Let's name $R$ the would-be cost of communication in {\MatchSubtree} if there were exactly $n$ active gadgets.

  We can see that $\Thr = P+Q+R$. If we have at least one half-active gadget, then the cost of communication in {\MatchSubtree} is greater than $R$, because we increase the number of 4-hop communications by at least one per each half-active gadget in comparison to solution where we have exactly $n$ active gadgets.
\end{proof}
...
\begin{proof}
Let's take an instance $I$ of $\TDPM$ and construct an instance $I'$
of $\RS(2)+\FP+\CC+\BW$ in the way described above.  We show that $I'$
has solution of cost $\leq \Thr$ if and only if $I \in \TDPM$ (there
exists a perfect 3D matching).
...
For each triple $t_1,\ldots, t_n$ in $\Sol$ we put $3$ nodes in leaves
of triple gadgets corresponding to those triples.  In each element
gadget (that corresponds to element $e$) we put $\deg(e)-1$ nodes. In
each element gadget there is only one leaf without the node placed in
it: this node contains the chunk replica that is processed in
{\MatchSubtree}.
...
produce a solution $\Sol$ to $I$ by taking triples that correspond to
active triple gadgets. Using lemma \ref{lem:full-or-empty} we conclude
that there are exactly $n$ active triple gadgets. By feasibility of
the $\Sol'$ we know that each chunk type was processed. From
observation \ref{obs:deg-min-one} we know that out of $\deg(e)$ chunk
types that correspond to $x\in A\cup B\cup C$ exactly one was
processed in the {\MatchSubtree}, and therefore each element of
$A\cup B\cup C$ is covered.
