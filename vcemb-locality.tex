...
We run min-cost max-flow algorithm. Then we go from each chunk following the non-zero flow path greedily (without returning to previous vertex) until we reach a VM. We output the pair as match. We decrease each edge on followed path by 1, remove chunk-vm pair from the tree and start again.
...
\begin{lemma}Algorithm produces perfect matching (Every chunk has matching pair) or the instance is infeasible
\end{lemma}
\begin{proof}
  \begin{enumerate}
