...
\begin{enumerate}
 ...
 This value corresponds to the cost of solution, where $n$ nodes are matched to $4$ chunks that are in distance: $0, 2, 2$ and $4$ to the node, and remaining $\Vms-n$ nodes are matched to $4$ chunks that are in distance: $0, 2, 2$ and $2$ to the node.
\end{enumerate}
...
To see this, note that the threshold value $\Thr$ corresponds to the cost of solution, where $n$ nodes has $1$ free match, $2$ neighbouring matches and $1$ long match at distance of $4$ hops, and remaining $\Vms-n$ nodes has $1$ free match and $3$ neighbouring matches assigned.
...
As at most $n$ nodes are spawned in \MatchSubtree{}, at least $\Vms-n$ nodes are spawned in the \CoverSubtree.
Assume then that $\Vms-n+i$ nodes spawned in the \CoverSubtree{} for non-negative $i$.
...
Using the fact that $\sum_e(\deg(e)-1) = 3\cdot t - 3\cdot n = \Vms - n$, by pidgeon-hole principle we conclude that indeed spawning $\Vms-n+i$ nodes in the \CoverSubtree{} results in at least $3\cdot i$ long matches.

Consider the configuration with $\Vms-n+i$ nodes spawned in the \CoverSubtree{}, and $n-i$ nodes spawned in the \MatchSubtree{}.
...
\begin{proof}
  ...
  The $n$ nodes placed in \TripleGadgets{} have $1$ free match and $2$ neighbouring matches to chunk replicas within the \TripleGadget{}, and $1$ long match of cost $4$ (to some \UnqGadget{}).
  The remaining $\Vms-n$ nodes placed in the \CoverSubtree{} have $1$ free match and $3$ neighbouring matches.
  In total, the cost incurred is $8\cdot n + 6\cdot (\Vms - n) = \Thr$.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.99\columnwidth]{reduction/overview2.pdf}
  \label{fig:red-cc}
  \vspace{-1em}
  \caption{Overview of the substrate network}
  \vspace{-1em}
...
\begin{footnotesize}
\begin{align*}
  \Thr  & = 2\cdot \binom{3}{2}\cdot n & \mbox{(over 2 hops in the \MatchSubtree{})}\\
        & + 4 \cdot \binom{3\cdot n}{2} & \mbox{(over 4 hops in the \MatchSubtree{})}\\
        ...
        & + 6\cdot\binom{m}{2} & \mbox{(over 6 hops)}
\end{align*}
\end{footnotesize}
