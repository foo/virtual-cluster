...
gadgets
  ...
  For each clause~$\mathcal{C}$, we generate~$3$ chunk types with~$2$
...
gadget.
We will then assign chunks to nodes in the following way: For each chunk type
...
processing those~$3$ chunk types. In this case, pick  
...
constraints, at most two of the chunk types, can be processed by nodes in the
...
\begin{proof}
  ...
  cost~$\leq \Thr$.
  By Observation~\ref{obs:nodes-match-cover}, we know that there are
 ...
 ~$\leq \Thr$.
  By Observations~\ref{obs:nodes-match-cover} and~\ref{obs:deg-min-one}, 
  we know that the communication cost of
  nodes in the {\CoverSubtree} is the same for every feasible solution
  (let's name it~$P$). We also know that the communication cost
