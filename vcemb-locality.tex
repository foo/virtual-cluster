...
a \emph{Min-Cost-Max-Flow} solution between super-source $\Source$ and super-sink $\Sink$ on the artificial graph $\Tree^*$ \maciek{Why don't we use min cost flow with setting the amount of flow upfront to number of chunk types? Min-cost-max-flow is more general problem.},.
...
a min-cost-max-flow solution \maciek{Why don't we use min cost flow with setting the amount of flow upfront to number of chunk types? Min-cost-max-flow is more general problem.}, e.g., using~\cite{mincostmaxflow-1,mincostmaxflow-2}.
...
(In addition, some paths may now have an integer allocation of 1.) We proceed inductively \maciek{Induction is proving tool, recursion is algorithm-design tool. Anyway, I would suggest to say ``We proceed until we construct the perfect matching''}.
...
dominant terms \maciek{term} in the asymptotic runtime is the flow computation.
Using the state-of-the-art min-cost-max-flow \maciek{Consider min-cost flow algorithms} algorithms~\cite{mincostmaxflow-1,mincostmaxflow-2}
...
The 2 replicas of each chunk type are
...
the $\MA$ resp.~$\MA+\BW$ problem\maciek{Consider removing the whole ``$\MA$ resp.'' part and only talk about $\MA+\BW$}.
...
in the
subtree rooted at $u$. \carlo{The bottom-up strategy ensures, that this works 
...
\maciek{Passive voice - consider rewrite.}
...
with the
remaining parts of $\Tree$\maciek{, which will inflict costs $\CostTrans$ per excess chunk on the uplink of root of $\Tree'$}. Similarly, if the processing capabilities exceed the
amount of
...
\carlo{the the assignments which generate the minimal bandwidth allocations 
described in
the proof of
...
\maciek{(Example)} Figure~\ref{fig:dynamic_motivation} shows an example: one
...
\textbf{Basic ideas.} For ease of presentation we will transform the 
...
we traverse the computed minimal-cost path backwards\maciek{I think that we lack detail here. We say that we traverse the path, but we do not mention that it is the path through the array of $f$ bottom-up computation}.

Concretely, the first argument to function $f$
is a subtree $\Tree'$, containing a given number of 
chunks-in-the-subtree $\ChunkCount(\Tree')$,
and the
...
summarized in the following figure: \maciek{TODO: reference the figure, add 
caption. I suggest to mark somehow the two basic problems in the graph - the 
problem variants that we proved the reduction for.}
...
a cost threshold $\Thr$. \carlo{The cost threshold will be met by all 
assignments which assigns all three chunks of each triple to a 
node which is collocated with one of the chunks. Assignments, which transport a 
chunk to a node in a different triple, will have a higher footprint, and are 
...
The gadget of each triple $t_i$ \maciek{Consider removing the name of the triple and refering as the triple} consists of an inner node (a router) and three leaves.
\item \emph{Chunks and chunk replicas:} For each element in $X$, $Y$ and $Z$,
 we create a chunk type
($3 \cdot k$ in total \maciek{This can be alternative place where we tell that $k$ is from the instance}). Every gadget \maciek{that correspond to triple}
contains three chunk replicas, corresponding to the elements of $t_i$\maciek{elements of the triple}.
...
create a gadget for each triple in $T$. The gadgets contain a chunk \maciek{of} the
type of the variable, for each variable that occurs in the triple\maciek{Consider reversing the order of the sentence, start with ``for each variable''}. The triple
...
via two hops, inflicting costs \maciek{unitary cost/cost 1} $1$ on each hop.
%\begin{figure}[htbp]
...
node must processes \maciek{must process} chunks that corresponds\maciek{correspond} to the triple, otherwise the
...
\end{lemma}
\begin{proof}
\carlo{Since $W$ is large enough, the $3\cdot k$ nodes have to be placed
...
problems, such as hitting set\maciek{We no longer have reduction from hitting set!} and 3-dimensional matching problems~\cite{3SC-hard} as well as flow problems~\cite{korte2002combinatorial}.
...
because the given valuation satisfied $\Phi$ \maciek{(because every clause was satisfied and chunks correspond to clauses)}.
