...
\begin{enumerate}
  ...
  \item The physical network consists of two parts: A {\MatchSubtree}
  and a {\CoverSubtree}. In the {\MatchSubtree} we put $2t$ gadgets:
  $t$ {\TripleGadgets} and $t$ {\UnqGadgets} (remember that $t$ is the
  number of triples in {\TDPM} instance). Cover subtrees consist of
  $n$ element gadgets.
  ...
  to the root of the cover subtree), and $4\cdot(t(e)-1)+1$ leaves.
\end{enumerate}

\paragraph{Chunk Placement}
The chunks are placed as follows:
\begin{enumerate}
  \item \emph{Chunks in triples subtree:} For each triple $t$ we put
