...
Figure~\ref{fig:summary}. In particular, we have shown that
...
already embeddings in two-level fat-trees are NP-hard (practically speaking:
...
\caption{Summary of results presented in this paper}
\label{fig:summary}
\end{figure}
...
\textbf{Construction.}
\begin{comment}
Given any formula $\Formula$ in Conjunctive Normal Form, we produce
a $\RS+\FP+\BW$ instance as follows. First, we construct a substrate tree, consisting of
a root and separate gadgets for each variable of $\Formula$, each of which
is a child of the root.
The gadget of variable $v$ has a root, $\root(v)$, and two children:
$\positive(v)$ and $\negative(v)$. Vertex $\positive(v)$ has $\clauses$
children: $v_1, v_2, \ldots, v_{\clauses}$. Vertex $\negative(x)$ has
$\clauses$ children: $\neg v_1, \neg v_2, \dots, v_{\clauses}$. Every
gadget has the same structure: the same height and the same number of
leaves. In fact, we will set the available bandwidth to be the
...
levels of edges in $\Tree_{\Formula}$: \emph{top}, \emph{middle} and \emph{bottom}. 
...
We set number of chunks to be equal to number of clauses, $\ChunkTypes =
...
The threshold $\Thr$ is calculated by the 
communication cost among nodes in
a certain solution to our instance. This solution is constructed by placing
...
spawn at most $cl(\Psi)$ virtual machines, according to the
Bandwidth Lemma. FIXME. 
