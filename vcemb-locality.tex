...
\maciek{We can have per-VM MA factor. When combined with FP we just generalize to per-server MA factor.}
...
can be reduced to $\RS+\MA+\BW$ (resp.~its subproblems).
\end{obs}
...
the set of chunks.
Concretely, we clone each node $\MaFactor$ (\maciek{Shouldn't ma factor be a function of a node?}) times,
...
$\MaFactor \cdot n_V \cdot \ChunkType$ \maciek{Again, ma factor is per node} many edges,
...
\CostTrans$. \maciek{Here we can generalize this lemma to different MA factors for different machines. I would suggest introducing ``processing power of the subtree function'', which would be a sum of number of cpus of VMs in the subtree.} \label{lemma:uplink}
\end{lemma}
\begin{proof}
  \maciek{I wouldn't start any proof with ``clearly''}
...
Combined with a simple postprocessing step, this approach can also solve $\MA+\BW$. The central idea of this extension \maciek{Change ``extension'' to ``problem variant''}, is
...
\maciek{This figure should be redone in Venn fashion. You can start by clean Venn, add the basis NP-complete problems ($\FP+\RS+\MA$ and $\FP+\RS+\CC$ in grey, then other variants in darker grey, then the middle problem in (almost) black. Alternative formulation is arrows on Venn diagram but I doubt that those will be readable.}
...
a cost threshold $\Thr$; solutions of higher costs are considered infeasible. \maciek{We would like to give the reader some intuitions about $\Thr$ value at this point. We can say that such amount is chosen so the VMs are not spread among too many gadgets.}
\end{itemize}

We will focus on problem instances where the server capacity is one unit (one node). \maciek{This comment is wrong. We can say something like that if we are restricting the set we are reducing from. But here we just set the server capacity to one unit, so it belongs to construction section, not to the assumptions before the construction.}
...
(k - 1) \cdot k$.\maciek{We can simplify it even further.}
%\maciek{If we update the model to take care of hosting
%  multiple VMs in one leaf, we should set this instance to allow only
...
\end{small}
\caption{
\maciek{I think that we would like to make a summary in one of two forms: (1) Venn diagram (but with sane colors) (2) list of problems groupped in circles like in my old summary.pdf. The tabular form is inferior.}
