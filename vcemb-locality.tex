...
\section{Excursion: A Detailed Study of\\Replica Selection Hardness}\label{ap:tworep}

We have seen that replica selection flexibilities can render embeddings computationally hard.
We will now have a more detailed look at this hardness result
...
There are not capacity constraints on links, and no extra, idling nodes.
...
$e\in X\cup Y\cup Z$. We define $n=|X|=|Y|=|Z|$. 
...
\item We construct $n$ chunk types named $u_1, \ldots, u_n$. 
Intuitively, these will be used to 
choose the $n$ triples of our solution.
\item \emph{Unique Chunks:} For each element $e\in X\cup Y\cup Z$, we 
will construct additional $3\cdot(\deg(e) - 1)$ \emph{unique chunks}. 
We call this set $U(e)$.
\end{itemize}
...
A matching subtree
  and a cover subtree. Matching subtrees consist 
  of two parts: $t$
  triple gadgets and unique gadgets. Cover subtrees consist of
  $n$ element gadgets.
\item \TripleGadget: For each triple, we create a subtree
  consisisting of four vertices: three leaves and one triple root. 
  We attach the root of the triple to the root of matching
  subtree.
...
\item \emph{Chunks in unique subtree:} We put chunks $u_1,\ldots, u_n$ in leaves of
  \UnqGadgets.
...
\textbf{Other properties of the instance.}
\begin{enumerate}
\item \emph{Multiple assignment:} We set the number of processed chunks by
  each machine to 4.
\item \emph{Number of machines:} We allow to spawn
  $\numNodes := n + \sum_{e}(\deg(e)-1)$ machines.
\item \emph{Threshold:}
...
\begin{obs}
We have one instance of each chunk type in \MatchSubtree.
\end{obs}
...
\textbf{The Reduction.}
\begin{enumerate}
  \item We put $n$  nodes in any leaf of such {\TripleGadgets} that were
  chosen in 3DPM solution. For each such node we match to it 3 chunks
  ...
  chunks from this Element Subtree to those nodes in any possible
  way.
  \item Such a solution has cost $\leq \Thr$ (easy to see by
  summing costs of transporting chunks to each node)
  \item Feasibility of the constructed solution: each chunk type was processed. Each node processes 4 chunks.
\end{enumerate}
...
\begin{theorem}
  Let's take any feasible solution $\Solution$ to the instance $I$ of VCEMB
  (as constructed above). If cost of $\Solution$ is at most $\Thr$, then no node
  is spawned in \UnqSubtree.
  \label{th:no-unique}
\end{theorem}

\begin{proof}
  Let's assume the contrary, that in feasible solution $\Solution$ at least
  one node spawned in \UnqSubtree. We show that in this case, the
  cost of solution $\Solution$ is greater than $\Thr$.

  Let's name $l$ the number of node that was spawned in $\Solution$ in Unique
  Subtree. We know that $1 \leq l \leq t$. We denote the set of nodes
  spawned in {\UnqSubtree} as $\SpawnedUnqSubtree$.
  ...
  most $\numNodes$ transportations are of cost $0$. Note that leaves of
  \UnqSubtree are separated from other leaves of the tree by at
  least $4$ edges.

  Cost of chunk transportation to nodes $v \in \SpawnedUnqSubtree$ is at least
  ...
  $\CostSol(S) \geq \CostEstimOne(l)) = 0 \cdot \numNodes + l \cdot 12 +
  (t-l)\cdot 4 + (4\cdot \numNodes - \numNodes - 4\cdot l - (t-l))\cdot 2$

  It can be verified that for each $l$ we have that
  $\CostEstimOne(l)) > \Thr$. To do so, we first show that
  $\CostEstimOne(l+1)) \geq \CostEstimOne(l))$, for $1\leq l \leq t-1$. We have
  that $\CostSol(S) \geq \CostEstimOne(1)) \geq \Thr$.
\end{proof}

\begin{theorem}
  Let's take any feasible solution $\Solution$ to the instance $I$ of VCEMB
  (as constructed above). If cost of $\Solution$ is at most $\Thr$, then
  exactly $n$ nodes spawn in \MatchSubtree.
  \label{th:np-balance}
\end{theorem}

\begin{proof}
  Let's assume the contrary, that is that the number of nodes $\SpawnedMatchSubtree$
  spawned in the \MatchSubtree, and $|\SpawnedMatchSubtree| \neq n$. We denote
  $l=|\SpawnedMatchSubtree|$. First, we use the theorem \ref{th:no-unique} to
  restrict the placement of nodes in \UnqSubtree. Then we consider
  cases:
  \begin{enumerate}
  \item $l \leq n$. In this case there are at least
    $u := 4 \cdot (n-l)$ chunks in the {\MatchSubtree} that are not
  ...
  \item More than $n$ nodes spawned in \MatchSubtree. In this case we
    use the fact that there are not enough nodess in {\CoverSubtree}, and
...
any leaf of it. We call nodes in active {\TripleGadgets} the \ActiveNodes. We
...
\item Note that in $\Solution$ only one node is spawned in Active \TripleGadget.
...
\item In each {\ElGadget} for element $e$, one chunk instance of
  ...
  $\Unmatched(e)$. Let's call $\Unmatched = \cup_e \Unmatched(e)$. We have that $|\Unmatched| = n$
\item Set $\Unmatched$ was covered by \ActiveNodes
\end{enumerate}
\end{proof}

\section{Two replicas without multiple assignment}


\subsection{The substrate network}
...
We use bandwidth constraints of form: $\Band(k) := k\cdot(\numNodes - k)$. Namely, we set bandwidth constraints of uplink of {\TripleGadgets} to be $\Band(3)$, and of uplink of {\ElGadget} for each element $e$ to be $\Band(\deg(e)-1)$. We also set the bandwidth on uplink of $\MatchSubtree$ to be $\Band(n)$ and on uplink of $\CoverSubtree$ to be $\Band(\sum_e (\deg(e)-1)$.


We set $\CostTrans = \infty$, and $\CostCom = 1$, and we host only one node per machine.

\subsection{Properties of the substrate network}
...
(work in progress)
\begin{enumerate}
\item on top level: we know that we place $\numNodes$ nodes in the subtree
\item we have $k$ children of root, each having bandwidth of $\Band(a_i)$
\item sum of $a_i$ is $n$
\item generalize to lower parts of the tree, where we have $l \leq \numNodes$ nodes to spawn (but still we control the total number of nodes, and $a_i$'s sum to $l$)
\end{enumerate}
\end{lemma}

\begin{proof}
\begin{enumerate}
\item show that only one solution to $\remainBw(s)$ exist
\end{enumerate}
\end{proof}
