...
Before we start the reduction, let us introduce two functions that will help us show that certain node placements are infeasible, as their cost exceeds the threshold.
 ...
 ~$\CostEstimOne(u) = u \cdot 12 + (t-u)\cdot 4 + (4\cdot n -
  n - 4\cdot u - (t-u))\cdot 2$, and 
$\CostEstimTwo(u) = 4\cdot t + (4\cdot
n - n - t)\cdot 2$, where $t$ is the number of triples and $n = |X| = |Y| = |Z|$ in a given instance of $\TDM$.
