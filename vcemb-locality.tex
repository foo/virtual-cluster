...
\carlo{Figure~\ref{fig:venn_matching} shows illustrates which problem 
combiantions can be solved by the presented matching approach.}
...
\subsubsection{Bandwidth correctness (moved from DP).}
...
This is a direct consequence of Lemma~\ref{}, as the amount of chunks and VMs 
in all subtrees is static, and all links are uplinks of subtrees (which 
possibly consist of only one node).
...
\carlo{We should point out, that all optimal assignment have exactly the same 
footpint (not only the sum, but on each individual link). This is a consequence 
of the lemmas in the DP part.}

\begin{proof}
 By contradiction. Assume a different assignment $\VmChunkAssignment'$ exists, 
with $\Cost(\VmChunkAssignment') > \Cost(\VmChunkAssignment)$.
\end{proof}
...
\maciek{Below there is emergency lemma in case that above lemma does not hold. 
In case above lemma is true it implies that below lemma is true.}
\begin{lemma}
...
\caption{To solve $\RS+\MA+\CC+\BW$, we have to find a cost minimal flow of 
value $4$ from $\Source$ to $\Sink$. The dotted link represents links, which 
have lower capacities due to $\CC$ - the dashed lines represent one unit of 
flow allocation in the cost optimal solution.}
\end{figure}
