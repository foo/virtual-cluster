...
Summing up the costs, we have:
  ...
  that $\CostSol \geq \CostEstimOne(1)) \geq \Thr$.
\end{proof}
...
\end{lemma}
\begin{proof}
  \maciek{TODO: introduce $\SpawnedMatchSubtree$. Why use $l$?}
  ...
  \item $\ell \leq n$. In this case there are at least
    ...
    \maciek{TODO: introduce $\CostEstimTwo$}

    We lower-bound the cost of $\Solution$ in the following way:
    $\CostSol \geq \CostEstimTwo(u) = 0\cdot \numNodes + 4\cdot t + (4\cdot
    \numNodes - \numNodes - t)\cdot 2$.
...
\maciek{TODO: say what to do to finish the proof}
