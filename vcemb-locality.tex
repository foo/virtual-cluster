...
\newcommand{\numNodes}{\textit{nodes}}
\newcommand{\UnqSubtree}{\textit{Unique Subtree}}
\newcommand{\MatchSubtree}{\textit{Matching Subtree}}
\newcommand{\SpawnedUnqSubtree}{\textit{nodes(\UnqSubtree)}}
\newcommand{\SpawnedMatchSubtree}{\textit{nodes(\MatchSubtree)}}
...
\item \emph{Element Gadget:} For each element $e \in X\cup Y\cup Z$, we
...
\textbf{The Reduction.}
\begin{enumerate}
  \item We put $n$  nodes in any leaf of such Triple Subtries that were
...
\begin{proof}
  ...
  one node spawned in Unique Subtree. We show that in this case, the
  ...
  spawned in Unique Subtree as $\SpawnedUnqSubtree$.
...
\begin{theorem}
  ...
  exactly $n$ nodes spawn in Matching Subtree.
  \label{th:np-balance}
\end{theorem}

\begin{proof}
  Let's assume the contrary, that is that the number of nodes $\SpawnedMatchSubtree$
  spawned in the Matching Subtree, and $|\SpawnedMatchSubtree| \neq n$. We denote
  $l=|\SpawnedMatchSubtree|$. First, we use the theorem \ref{th:no-unique} to
  restrict the placement of nodes in Unique Subtree. Then we consider
  cases:
  \begin{enumerate}
  ...
  \item More than $n$ nodes spawned in Matching Subtree. In this case we
    use the fact that there are not enough nodess in Cover Subtree, and
...
above. We call the Triple Gadget \textit{active}, if we have a node spawned in
any leaf of it. We call nodes in active Triple Gadgets the \ActiveNodes. We
...
\item Note that in $\Solution$ only one node is spawned in Active Triple Gadget.
