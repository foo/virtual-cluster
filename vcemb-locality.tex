...
reservations, see also~\cite{oktopus,proteus}.
...
We now introduce our model more formally. Our model consists of three fundamental parts: (1) the substrate network (the servers
...
\textbf{Remark.}
We conclude this section by remarking that our model does not explicitly describe
the process of 
copying back the final results from the
reducers to the HDFS replica. This step offers many flexibilities
and can simply be determined beforehand.~\cite{endpoint}
...
\caption{Variants solved by matching approaches.\maciek{There are still BW and BW+MA left to mark} }
...
introduced above.
In general, we can refer to the algorithms presented in this section 
...
Perfect
Matching} on the bipartite graph~\cite{gabow_scaling_algorithm}:
...
graph. The postprocessing time scales linearly in the number of edges in the substrate graph.
...
We encode a $\TDM$ instance as an $\FP+\RS+\MA$ instance as follows:\maciek{Those basic ideas are shared between two proofs. Maybe we should put those in the intro? But it is good the way it is also.}
 ...
 independently in which leaf the node is mapped\maciek{independently of which leaf the node is mapped to}.
...
node which is collocated with one of the chunks. Assignments which communicate a
