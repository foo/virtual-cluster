...
We can also solve $\CC+\MA+\BW$ with our matching approach. The algorithm works 
similar as the previous one: in fact the construction is the same - we entirely 
ignore the bandwdith constraints until we computed a solution to minimum weight 
perfect matching. Once we have the solution, we can check whether any bandwdith 
constraints are violated. If none are violated, we obviously found the cheapest 
overall solution. We will now show, that the problem is infeasible, if the 
solution which ignored the bandwdith constraints exceeds the constraints on at 
least one link. Before we can proof the infeasibility of the instances, we will 
introduce a lemma, to better understand the \emph{locality} of optimal 
assignments.

\begin{lemma}
Given a problem instance with $\MA$ and subtree $\Tree'$, containing $n$ 
