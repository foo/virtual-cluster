...
to classic optimization problems, such as matching and
...
While these flexibilities
can in principle be exploited to improve resource sharing,
the underlying embedding problems are non-trivial,
and have been studied intensively over the last years.

A very popular abstraction for for batch-processing applications is the \emph{virtual cluster}.~\cite{oktopus}
As batch-processing applications such as often generate significant
network traffic (namely during the so-called shuffle phase)~\cite{amazonbw},
in order to avoid interference in the underlying physical network and in order to provide a predictable
application performance,
it is important to provide performance isolation and bandwidth guarantees between virtual machines.~\cite{talk-about}
Accordingly, the virtual cluster not only defines a set of virtual machines with resource guarantees,
but also an interconnect with (symmetric and pair-wise) bandwidth guarantees, the \emph{virtual network}.
In order to provide resource-efficiency, several algorithms have been proposed to map virtual machines as close as possible
to each other in the datacenter, which minimizes communication costs.~\cite{oktopus,proteus}

However, existing embedding algorithms often ignore an important aspect: the fact that the input to the batch-processing applications,
the data, are stored in a distributed file system or database, on different servers. Therefore, in order to minimize
costly data transmissions, an embedding algorithm should be \emph{data locality aware},
and allocate (or \emph{embed}) the compute units close to the chunk locations. Moreover, in case of redundant storage (batch processing
applications often provide a 3-fold redundancy), the algorithm should exploit flexibilities in
the \emph{replica selection}.

\subsection{Our Contributions}

This paper initiates the study of data-locality and replica aware embedding problems in virtualized datacenters.
In particular, we decompose the optimization problem into different fundamental models, in terms of
replica selection, flexible virtual machine embedding, or capacity constraints, and ask questions such as:
\begin{enumerate}
\item Can replicas selected and assigned optimally in polynomial time?

\item Can virtual machines be embedded optimally in polynomial time?

\item Can the joint optimization problem of replica selection and virtual machine placement be solved in polynomial time?
\end{enumerate}
...
replica selection and embedding), and also highlight
computational hardness limitations. We highlight interesting connections to classic algorithmic
problems such as matching and flow problems. In terms of hardness results,
we provide reductions from 3-D matching, SAT and coverage problems; interestingly,
while it is well-known that (unsplittable) multi-commodity flow
problems are NP-hard in capacitated networks, our hardness results also hold for uncapacitated
...
to batch-processing applications. Figure~\ref{fic:overview} gives on overview of the model. TODO CARLO
...
(3) the virtual network (the virtual machines and the logical networking connecting the machines to each other
...
and a set $E$ of links. Concretely, we assume that the inter-connecting network forms an (arbitrary) tree,
...
The data is stored in a distributed manner; this distribution is given and not subject to optimization.
The input data consists of different \emph{chunk types} $\{\achunk_1, \ldots, \achunk_k\}$,
where for each chunk type $\achunk_i$,
...
which server it is stored. It is sufficient to process one replica, and we will refer to this
...
Each node $v$ can be placed (or, synonymously, \emph{embedded}) on a server; this placement can be subject
to optimization.
...
However, in order to avoid stragglers, we require that each nodes needs to process the same number of chunks.
Moreover, in order to ensure a predictable performance, both the connection to the chunks
as well as the interconnection between the virtual machines may have to provide certain
minimal bandwidth guarantees; we will refer to the first type of network as the \emph{access
...
the \emph{resource footprint}: the overall bandwidth allocation in the datacenter. (Note that
...
In the number of chunk types is larger than the number of virtual machines,
...
where bandwidth needs to be reserved for the inter-connect, by $\CC$.
...
a shuffling operation is performed, phase where the results from the mappers
...
two virtual machines ($\CostCom$); in practice, for applications with a large so-called
\emph{mapping ratio}, $\CostCom<\CostTrans$. Finally, note that it is frequently assumed
...
$\MaFactor$ replica types, and collect all copies of a given chunk type in a
single %$\ChunkType$
...
systems for the processing and analysis of big data (e.g.~\cite{nodb,mapreduce,shark}). The model studied in 
...
These applications
generate large amounts of network traffic~\cite{talk-about,amazonbw,orchestra},
and over the last years, several systems have been proposed which provide
a provable network performance, also in shared cloud environments, namely by supporting explicit 
...
between the virtual machines or compute units.
...
The most popular abstraction for batch-processing applications today is the \emph{virtual cluster},
introduced in the Oktopus paper~\cite{oktopus}, and later studied by many other researchers~\cite{talk-about,proteus}.
...
(e.g., motivated by wide-area networks).~\cite{ammar,zhu06,simannealing,turner,infocom2009,ucc12mip}

From a theoretical perspective, the virtual network embedding problem can be seen as a generalization
of classic VPN graph embedding problems~\cite{gupta2001provisioning,Goyal2008},
...
e.g., with sublogarithmic approximation ratios~\cite{mla-feige}. 
