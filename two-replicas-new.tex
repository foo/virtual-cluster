...
In this way the upper-bound on number of nodes always exceeds the number of leaves of any subtree in which we would like to have $k$ nodes spawned.
...
\begin{enumerate}
 ...
 In total, we place $\deg(e) - 1$ replicas, one per each leaf of the gadget.
\end{enumerate}
...
\begin{lemma}
 ...
 ~$\Band(k)$. Assume that no chunk transportation is allowed
  ...
  name it~$s$) satisfies~$s \leq k \vee \Vms-s\leq k$, and~$s \leq l$.
