...
\textbf{Construction.}
Let's take any instance~$I_{\TDPM}$ of~$\TDM$ and construct a~$\RS(2)+\MA(4)+\FP$ instance~$I_{\VCEMB}$ the way described in the remainder of this section.
...
\begin{enumerate}
  ...
  {\MatchSubtree} consists of a {\TripleGadget} per each triple $\tau\in T$ and $n$
  ...
  {\UnqGadgets} far from leaves of other \UnqGadgets. Note that Unique Gadgets form a comb.
...
\begin{enumerate}
  \item \emph{Multiple assignment:} We set the multi-assignment factor (number of processed
  chunks by each node) to $4$.
  \item \emph{Number of nodes:} We allow to spawn
 ~$\numNodes = n + \sum_{e}(\deg(e)-1)$ nodes.
  \item \emph{Threshold:} We set the following threshold:~$\Thr = 8\cdot n + 6\cdot\sum_{e}(\deg(e)-1)$
\end{enumerate}

\textbf{The reduction.}

\maciek{TODO: rewrite start}
