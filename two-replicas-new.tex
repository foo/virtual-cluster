...
For each $e\in X\cup Y\cup Z$, by $T_e$ we denote the set of all triples that contain element $e$, and $\deg(e) = |T_e|$.
...
\begin{enumerate}
 ...
 We put $\deg(e)$ replicas of chunks $ch_2(\tau, e)$ for each $\tau \in T_e$.
 Additionally, we place all the replicas from set $\UniqueE$.
 In total, we place $4\cdot \deg(e) + 1$ replicas, one per each leaf of the gadget.
\end{enumerate}
