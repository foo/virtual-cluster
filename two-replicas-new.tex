...
The reduction unfolds in two stages.
...
Our goal is to show that in every feasible solution, exactly $n$ \TripleGadgets{} are active, and hence the $\TDPM$ solution can be constructed from the triples that correspond to active \TripleGadgets{}.

In the constructed $\RS(2)+\MA+\FP$ solution, chunks can be matched to nodes at distance $0, 2, 4$ and $6$.
We call matches at distance $0$ the free matches, the matches at distance $0$ or $2$ the short matches, and the matches at distance $4$ or $6$ the long matches.
Notice that at most $|V|$ matches can be free, as there exist exactly one chunk replica in each leaf, and the node has to be collocated with the replica for the match to be free.
Also notice that proving the existance of more than $n$ long matches is sufficient to show that the instance is infeasible.

We start by observing that each node spawned in the \MatchSubtree{} results in at least one long match.
Each node has $4$ chunk replicas assigned.
For each node spawned in the \MatchSubtree{}, by the construction of the tree, the node has at most $3$ leaves in distance at most $2$, hence at least one of the matches is long.

Spawning more than $n$ nodes in the \MatchSubtree{} results in more than $n$ long matches, which results in infeasibility of the solution.
Hence, every feasible solution has $|V|+n+i$ nodes spawned in the \CoverSubtree{} for non-negative $i$.
Now, we argue that such node configuration results in $3\cdot i$ long matches.
\maciek{TODO}

Consider the configuration with $|V|+n+i$ nodes spawned in the \CoverSubtree{}.
Such configuration results in $2\cdot i + n$ long matches, where $3\cdot i$ long matches come from the excessive nodes spawned in the \CoverSubtree{}, and $n-i$ long matches come from $n-i$ nodes in the \MatchSubtree{}.
Hence we deduce that $i = 0$, as otherwise the number of long matches exceeds $n$.

Now, we deduce that no node spawned in the \UnqSubtree{}. \maciek{TODO}

Hence, we conclude that exactly $n$ nodes spawn in the \TripleGadget{}.
Combine this with the fact that at most one node spawned in each \TripleGadget{}, and we conclude that exactly $n$ \TripleGadgets{} are active.
