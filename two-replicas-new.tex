...
\begin{proof}
  In the constructed $\RS(2)+\MA+\FP$ solution, chunks can be matched to nodes at distance $0, 2, 4$ or $6$.
  We call matches at distance $0$ the \emph{free matches}, the matches at distance $0$ or $2$ the \emph{short matches}, and the matches at distance $4$ or $6$ the \emph{long matches}.
...
The threshold value $\Thr$ corresponds to the cost of solution, where $n$ nodes are matched to $4$ chunks that are in distance: $0, 2, 2$ and $4$ to the node, and remaining $|V|-n$ nodes are matched to $4$ chunks that are in distance: $0, 2, 2$ and $2$ to the node.
...
Using the fact that $\sum_e(\deg(e)-1) = 3\cdot t - 3\cdot n = |V| - n$, by pidgeon-hole principle we conclude that indeed spawning $|V|-n+i$ nodes in the \CoverSubtree{} results in $3\cdot i$ long matches.
