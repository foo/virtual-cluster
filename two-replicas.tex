...
\maciek{We need to introduce the idle machines extension, that is
  needed for reduction to work. We can also tell the reader that our
  dynamic program works for this extension (f(T,x,idle)), and even
  with MA (with assumption that each VM either EMPTY or FULL); NP-hardness
  proofs holds with setting $idle=0$; our flow algorithm does not
  work, though}
...
computed in such a way does not contain transportation cost.
...
variable gadgets; however, now we place distinct chunks $c^1_i, c^2_i,
c^3_i$ instead of
