...
\author{Carlo Fuerst$^{1,*}$, Maciej Pacut$^{2,*}$, Paolo Costa$^3$, Stefan Schmid$^4$\\
{\small $^1$ TU Berlin, Germany; $^2$ University of Wroclaw, Poland; $^3$ TU Berlin \& T-Labs, Germany; $^1$ Microsoft Research, UK} \\ {\small $^*$ authors contributed equally to this work}}
...
two virtual machines ($\CostCom$); \carlo{REMOVE:}in practice, for applications with a large
...
(2) the to be processed input (\carlo{divided into data chunks}), and
...
\carlo{This appears broken. New attempt: Our goal is to develop algorithms to embedd virtual clusters on the substrate network. Ideally requests are not rejected, if they can be accepted without violating resouces capacities, and the algorithms minimize the \emph{resource footprint}: the amount of resources which have to be dedicated to a virtual cluster, in order to realize it's guarantees.}
...
and bandwidth constraints ($\BW$), as described in the following. \carlo{Strong Claim - no citation. We should change this: While it is possible to extend this to additional aspects such as latency constraints, we focus on these five apsects as they can have a big impact on todays batch processing applications.}
...
each node needs to be assigned multiple chunks. We will refer to such a scenario by $\MA$. \carlo{Since all nodes are identical and no additional information regarding the chunks is available at request time, we assume that each node will process an identical integer number of chunks $\MaFactor = \tau / n_V$.} %We will refer to such a scenario by $\MA$,
...
where bandwidth needs to be reserved also for the inter-connect, by $\CC$. \carlo{The node interconnect is modelled as a full graph, to account for the all to all communication patterns of batch processing applications such as MapReduce.} 
...
\carlo{Previous was inprecise - New Attempt: In particular, we
...
many problems can be solved in polynomial time. However, we have also
shown that several embedding problems are NP-hard already in two-
and three-level trees---a practically relevant result given today's datacenter topologies~\cite{fattree})---and even if the the number of replicas is bounded by two.
\begin{table}
\tiny
\bgroup
...
\textbf{Acknowledgments.} We would like to thank our shepherd Kihong Park.
