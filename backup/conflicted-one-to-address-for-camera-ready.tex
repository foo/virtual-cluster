...
assignment of chunks (the atomic data units), replica selection, and flexible virtual machine placement, and answer questions such as:
\begin{enumerate}
...
\caption{Variants solved by flow approach.\maciek{We added 0 cost to this diagram. I don't know if it's bad.}}
\vspace{-1em}
\label{fig:venn_flow}
\end{wrapfigure}
...
$\RS+\MA+\CC$ and $\MA+\CC+\BW$ which can also be solved with
\begin{wrapfigure}{r}{0.5\columnwidth}
\vspace{-1em}
\includegraphics[width=0.48\columnwidth]{figs/venn_matching.pdf}
\caption{Variants solved by matching approaches.\maciek{In both DP and flow we added 0 cost to the picture. Here we don't.}}
\vspace{-1em}
...
\caption{Variants solved by dynamic programming approach.\maciek{Here we seletively added some 0 cost variants, but skipped on FP+RS. In its current state it really shows that limit of DP is RS, but the diagram itself is wrong.}}
\vspace{-1em}
\label{fig:venn_flow}
\end{wrapfigure}
...
As usual in dynamic programs, we define, over the structure of the tree, an inductive \maciek{recursive formula} formula $f$ for
...
(given recursively\maciek{given inductively}) of placing $r$ nodes in
...
\vspace{-1em}
\end{wrapfigure}
\textbf{Remark: Simple Problems.}\maciek{Why did we put simple problems in dynamic programming section? We should put it into new ``summary'' section where we also sum up the runtimes.}
...
$\RS$ and $\BW$ (but without $\MA$ or\maciek{and} $\CC$) can easily be solved by mapping
...
summarized as follows:\maciek{Consider scaling down the figure. Also, I think that sometimes this figure is displayed wrong - arrows are missing some parts. It happens so in some compilations but I cannot find the reason for that.}\\
\begin{figure}[h!]
...
($3 \cdot k$ in total). Every gadget contains three chunk replicas, \maciek{we place one replica per node - we generalized chunks per nodes and we have to say that}
...
(k - 1) \cdot k$, to verify whether a solution achieves this \maciek{achieves smaller cost}, transforming
...
or the \emph{negative} subtree.\maciek{Mentioned lemma does not say anything about VMs sticking to pos or neg, only about saturation of gadgets.}} Then we can compute a positive valuation by
=======
Let us take any solution $\Sol$ to $\RS(2)+\FP+\BW$ of cost $\leq \Thr$.
