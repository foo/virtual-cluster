...
\carlo{We should find a better environment here.}
\textbf{Observation: $\RS+\MA+\CC + \BW$ degenerates to $\RS+\MA+\BW$}
In the absence of node placement flexibilities (no $\FP$),
...
selection and assignment. Thus $\RS+\MA+\CC + \BW$ problems boil down to 
$\RS+\MA+\BW$.

This implies, that it is sufficient to solve $\RS + \MA$, if we want 
to solve $\RS+\MA+\CC$. In order to solve $\RS+\MA$, we construct a bipartite 
...
Many algorithms solve the minimum weith perfect matching problem on 
bipartite 
...
We will now try to improve the complexity for $\MA+\CC$ by 
leveraging a certain locality property, which (as we will show) is present in 
all optimal assignments for these instances. In addition we will show that 
instances of 
$\MA+\CC+\BW$ can be solved with the same appraoch, combined with 
postprocessing in oder to verify that the soltuion does not violate bandwidth 
constraints.



From the observation in the last section, we know that it is sufficient to show 
a solution to $\MA$, from which it is trivial to generate a solution for 
$\MA+\CC$.
...
\textbf{Construction} We will now describe, how to construct a \emph{local} 
assignment in linear time. At the heart of our construction, lies a FIFO queue, 
which determines the order in which vertices of the substrate graph are 
traversed. At initialization, the queue contains all leaves of the substrate 
graph. At each node, we keep two lists: one contains known and unmatched 
chunks, 
the other known and unmatched nodes. The lists are initilized with $\emptyset$

Once a leaf $l$ in the substrate graph is chosen for processing, we add any 
nodes or chunks which are located on $l$ to the corresponding lists. As long as 
...
other children, of the parent of $l$ are allready processed. If they are, we 
add 
the parent of $l$ to the FIFO queue.
...
Subsequently we add the parent of $v$ to the FIFO queue, if all other chrildren 
of the parent of $v$ have allready been processed.
