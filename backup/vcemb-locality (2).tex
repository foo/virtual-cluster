...
\item We construct $n$ additional chunk types named $u_1, \ldots, u_n$.
  Intuitively, we want $n$ nodes to process one of these chunks each
...
A \emph{matching subtree}
  and a \emph{cover subtree}. Matching subtrees consist 
  of two parts: $t$
...
we place the chunks
  $t(e)\cup U(e)$ at the leaves of each element subtree.
\end{enumerate}
...
uses two replicas for each chunk type.
We can make the following observations.

\begin{obs}
The construction fulfills the following properties:
\begin{enumerate}
\item There is one instance of each chunk type in \MatchSubtree.

\item There is one instance of each chunk type in the elements subtree.
...
\begin{enumerate}
  \item We put $n$  nodes in any leaf of the {\TripleGadgets} that were
...
\begin{proof}
  Let's assume the contrary: a feasible solution $\Solution$ 
  has at least
  one node spawned in \UnqSubtree. We show that in this case, the
  cost of solution $\Solution$ is greater than $\Thr$.

  Let $\ell$ denote the number of nodes that was spawned in $\Solution$ in 
  the Unique
...
\begin{theorem}
  Let's take any feasible solution $\Solution$ to the instance $I$ of $\FP+\RS(2)+\MA(4)$
...
\begin{proof}
  Let's assume the contrary, that the number of nodes $\SpawnedMatchSubtree$
  spawned in the \MatchSubtree, and $|\SpawnedMatchSubtree| \neq n$. We denote
  $\ell=|\SpawnedMatchSubtree|$. First, we use the Theorem~\ref{th:no-unique} to
...
\textbf{Construction of the matching problem solution.}
